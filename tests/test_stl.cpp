 ...
 * */
template <typename T>
void print_vec(string s, const vector<T>& v) {
    cout << s << endl;
...
template <typename T>
void print_list (string s, const list<T>& lst) {
    cout << s << endl;
    for (const T& i : lst) {cout << i << ' ';}
    cout << endl;
}
/*
...
void print(string s, const container& C) {
    cout << s << endl;
...
public:
    IsOdd(int num) : n(num) {};
    bool operator () (int num) {return (n % 2) != 0;}
};
...
int main() {
    ...
    list<int>::iterator odd_iter3 = find_if(ilist.begin(), ilist.end(), [](int n) {return n%2 != 0;});
